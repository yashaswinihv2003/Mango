# ============================================================
# PROFESSIONAL FARMER DECISION SUPPORT SYSTEM (FAST VERSION)
# Road Distance + Optimized OSRM Calls
# ============================================================
...
def get_road_distance(v_lat, v_lon, d_lat, d_lon):
    try:
        url = (
            f"https://router.project-osrm.org/route/v1/driving/"
            f"{v_lon},{v_lat};{d_lon},{d_lat}?overview=false"
        )
...
if run:
    ...
    if df_all.empty:
        st.error("No facilities available for selected variety.")
        st.stop()
    ...
    for _, row in df_all.iterrows():
        ...
        km = get_road_distance(v_lat, v_lon, row["Lat"], row["Lon"])
    ...
    df_top10 = df.sort_values("Distance_km")

    st.subheader("Top Options (Road Distance Based)")
