# ============================================================
# 🥭 FARMER’S MANGO PROFIT NAVIGATOR – FINAL OPTIMIZED VERSION
# Fast Loading + Distance on Map + Icons + Highlighted Route
# ============================================================
...
# ================= LOAD ONLY VILLAGES FIRST (FAST START) =================
@st.cache_data
def load_villages():
    df = pd.read_csv("Village data.csv")
...
if st.session_state.run:
    ...
    with st.spinner("Loading market data..."):
        ...
        for df in [prices, geo, processing,
    ...
    for _,row in df_top.iterrows():
        ...
        route_info = next((r for r in routes if r["Name"]==row["Name"]), None)

        if route_info:
            road_path = route_info["coords"]
            dist_km = route_info["Distance"]
            ...
            folium.Marker(
                [row["Lat"],row["Lon"]],
                popup=f"{row['Name']}<br>Distance: {dist_km} km<br>Rank: {row['Rank']}",
                icon=icon
            ).add_to(m)
